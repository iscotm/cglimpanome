...
import { Plus, Search, ChevronRight, User, Phone, Mail, X, FileText, MessageSquare, Pencil, Trash2, MoreVertical, Edit } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
                        ...
                        <div className="flex justify-end items-center gap-3">
                          ...
                          <div className="relative">
                            ...
                            {activeMenuId === client.id && (
                              <div className="absolute right-0 top-full mt-2 z-50 w-36 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                ...
                <div key={client.id} className="p-4 bg-white hover:bg-slate-50 transition-colors active:bg-slate-100">
                  <div className="flex justify-between items-start mb-3">
                    ...
                    <div className="relative">
                      ...
                      {activeMenuId === client.id && (
                        <div className="absolute right-0 top-full mt-2 z-50 w-40 bg-white rounded-2xl shadow-xl border border-slate-100 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right">
                          <div className="p-1.5" onClick={(e) => e.stopPropagation()}>
                            <button
                              onClick={(e) => { e.stopPropagation(); handleEdit(client); setActiveMenuId(null); }}
                            ...
                            <button
                              onClick={(e) => { e.stopPropagation(); handleDelete(client.id, client.name); }}
                              className="w-full flex items-center gap-3 px-4 py-3 text-sm font-bold text-slate-700 hover:bg-rose-50 hover:text-rose-600 rounded-xl transition-colors"
                            >
      ...
      <DeleteConfirmationModal
        ...
        description={`Esta ação é irreversível. O cliente "${clientToDelete?.name}" será removido permanentemente, juntamente com todos os seus contratos e pagamentos.`}
      />
    </div>
  );
};
